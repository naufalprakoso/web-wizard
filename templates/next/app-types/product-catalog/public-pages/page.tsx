...
import type { Category, Product, ProductCatalogContent } from "@/lib/app-type/cms/schema";
  ...
  return (
    <>
      ...
      <main>
        <section className="bg-white px-4 py-5 md:py-8">
          <div className="mx-auto grid max-w-[1480px] gap-5 xl:grid-cols-[1.35fr_0.65fr]">
            <div className="relative min-h-[560px] overflow-hidden rounded-[32px] bg-zinc-950 text-white shadow-[0_30px_90px_rgba(17,17,17,0.16)]">
              <img
                src={content.heroImageUrl || productCatalogDefaultContent.heroImageUrl}
              ...
              <div className="relative flex min-h-[560px] max-w-xl flex-col justify-between p-6 md:p-10 lg:p-12">
                <div className="flex flex-wrap gap-3 text-[11px] font-black uppercase tracking-[0.24em]">
                  <span className="rounded-full bg-white px-4 py-2 text-zinc-950">{content.campaignLabel}</span>
                  <span className="rounded-full border border-white/30 bg-white/10 px-4 py-2 text-white backdrop-blur">{content.campaignPeriod}</span>
                </div>
                <div>
                  <p className="text-sm font-black uppercase tracking-[0.28em] text-white/70">{content.campaignEyebrow}</p>
                  ...
                  <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                    <ButtonLink href="#products" variant="secondary" className="rounded-full">{content.primaryCtaLabel}</ButtonLink>
            ...
            <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-1">
              {content.sideBanners.map((banner) => <CampaignCard key={`${banner.eyebrow}-${banner.title}`} {...banner} />)}
            </div>
          </div>
        </section>
        ...
        <section id="categories" className="border-y border-zinc-200 bg-white py-10 md:py-12">
          <div className="mx-auto max-w-[1480px] px-4">
            <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
              <div>
                <p className="text-xs font-black uppercase tracking-[0.28em] text-[#f97316]">{content.categoryEyebrow}</p>
                <h2 className="mt-3 text-3xl font-black leading-tight text-zinc-950 md:text-4xl">{content.categoryTitle}</h2>
              </div>
        ...
        <ProductRail title={content.topDealsTitle} products={heroProducts} ctaLabel={content.whatsappCta} />

        {content.promoTiles.length > 0 ? (
          <section className="bg-white px-4 py-8">
            <div className="mx-auto grid max-w-[1480px] gap-5 lg:grid-cols-3">
              {content.promoTiles.map((tile) => <PromoTile key={`${tile.title}-${tile.href}`} {...tile} />)}
            </div>
          </section>
        ) : null}

        {accessoryProducts.length > 0 ? <ProductRail title={content.accessoryRailTitle} products={accessoryProducts} ctaLabel={content.whatsappCta} /> : null}

        <section id="products" className="border-y border-zinc-200 bg-[#f8f5ef] py-14 md:py-16">
          <div className="mx-auto max-w-[1480px] px-4">
            <div className="grid gap-6 lg:grid-cols-[0.75fr_1.25fr] lg:items-end">
              <div>
                <p className="text-xs font-black uppercase tracking-[0.28em] text-[#f97316]">{content.finderEyebrow}</p>
                <h2 className="mt-3 text-3xl font-black leading-tight text-zinc-950 md:text-5xl">{content.finderTitle}</h2>
              </div>
              <p className="max-w-2xl leading-8 text-zinc-600 lg:justify-self-end">{content.finderDescription}</p>
            </div>
            <div className="mt-8">
        ...
        {clothingProducts.length > 0 ? <ProductRail title={content.clothingRailTitle} products={clothingProducts} ctaLabel={content.whatsappCta} /> : null}
        ...
        <BrandStrip items={content.brandStrip} />
        <ServiceStrip items={content.serviceBenefits} />
...
function CampaignCard({ eyebrow, title, body, ctaLabel, href, imageUrl, imageAlt, tone }: ProductCatalogContent["sideBanners"][number]) {
  return (
    <a href={href} className={`group relative grid min-h-[270px] content-between overflow-hidden rounded-[32px] border border-zinc-200 ${toneClass(tone)} p-6 transition hover:-translate-y-1 hover:shadow-[0_24px_80px_rgba(17,17,17,0.12)]`}>
      {imageUrl ? <img src={imageUrl} alt={imageAlt} className="absolute inset-0 h-full w-full object-cover opacity-30 transition duration-300 group-hover:scale-105" /> : null}
      ...
      <span className="relative inline-flex w-fit rounded-full bg-zinc-950 px-5 py-3 text-sm font-black text-white transition group-hover:bg-[#f97316]">{ctaLabel}</span>
    </a>
  );
}
            ...
            <a key={product.id ?? product.name} href={`/products/${product.id ?? encodeURIComponent(product.name)}`} className="group overflow-hidden rounded-[26px] border border-zinc-200 bg-white shadow-[0_18px_60px_rgba(17,17,17,0.05)] transition hover:-translate-y-1 hover:border-zinc-950 hover:shadow-[0_24px_80px_rgba(17,17,17,0.1)]">
              ...
              <div className="p-4">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span className="text-xs font-black text-amber-500">{product.rating.toFixed(1)} · {product.soldCount.toLocaleString()} sold</span>
                </div>
...
function PromoTile({ title, ctaLabel, href, imageUrl, imageAlt, tone }: ProductCatalogContent["promoTiles"][number]) {
  return (
    <a href={href} className={`group relative min-h-[250px] overflow-hidden rounded-[30px] ${toneClass(tone)} p-6 shadow-[0_18px_60px_rgba(17,17,17,0.06)] transition hover:-translate-y-1`}>
      {imageUrl ? <img src={imageUrl} alt={imageAlt} className="absolute inset-0 h-full w-full object-cover opacity-35 transition duration-300 group-hover:scale-105" /> : <div className="absolute bottom-0 right-8 h-[76%] w-[36%] rounded-t-full bg-white/50 shadow-2xl backdrop-blur" />}
      <div className="relative flex min-h-[202px] flex-col justify-between">
        ...
        <p className="text-sm font-black uppercase tracking-[0.18em] text-zinc-950 transition group-hover:text-[#f97316]">{ctaLabel}</p>
      </div>
    </a>
  );
}
...
function ServiceStrip({ items }: { items: ProductCatalogContent["serviceBenefits"] }) {
  if (items.length === 0) return null;
  return (
...
function toneClass(tone: string) {
  const tones: Record<string, string> = {
    rose: "bg-gradient-to-br from-rose-100 via-pink-50 to-stone-100",
    violet: "bg-gradient-to-br from-violet-100 via-purple-50 to-stone-100",
    orange: "bg-gradient-to-br from-orange-100 via-rose-50 to-stone-100",
    blue: "bg-gradient-to-br from-blue-100 via-slate-50 to-stone-100",
    purple: "bg-gradient-to-br from-purple-100 via-pink-50 to-stone-100",
    green: "bg-gradient-to-br from-emerald-100 via-stone-50 to-zinc-100"
  };
  return tones[tone.toLowerCase()] ?? tones.orange;
}
